...
- Live assistant deltas and reasoning in gateways, plus stable-ID tool
  start/completion events for structured clients such as the TUI. The
  completion-only history projector uses the same IDs, so resumed sessions
  hydrate the tool cards shown during the turn.
