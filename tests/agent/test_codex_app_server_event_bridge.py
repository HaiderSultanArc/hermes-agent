...
class TestStructuredToolDispatch:
    def test_command_lifecycle_uses_projector_stable_id(self):
        agent = _make_stub_agent()
        bridge = make_codex_app_server_event_bridge(agent)
        started = {
            "type": "commandExecution",
        ...
        expected_id = "codex_exec_exec-structured"
        expected_args = {"command": "echo hi", "cwd": "/tmp"}
        agent.tool_start_callback.assert_called_once_with(
            expected_id, "exec_command", expected_args
        )
        agent.tool_complete_callback.assert_called_once_with(
            expected_id, "exec_command", expected_args, "hi\n"
        )

    def test_broken_progress_callback_does_not_hide_structured_callbacks(self):
        agent = _make_stub_agent()
        agent.tool_progress_callback.side_effect = RuntimeError("display failure")
        bridge = make_codex_app_server_event_bridge(agent)
        item = {
            "type": "dynamicToolCall",
            "id": "dyn-structured",
        ...
        bridge(_item_completed({**item, "success": True}))
